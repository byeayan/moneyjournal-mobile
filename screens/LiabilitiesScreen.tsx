...
export default function LiabilitiesScreen() {
  ...
  const [showAllLiabilityPayments, setShowAllLiabilityPayments] = useState(false);
  const [dismissedRecentPaymentIds, setDismissedRecentPaymentIds] = useState<string[]>([]);
  ...
  const liabilityPaymentRows = useMemo(
    () =>
      [...liabilityPayments]
        .filter((entry) => !dismissedRecentPaymentIds.includes(entry.id))
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
        .map((entry) => {
        const liability = liabilities.find((item) => item.id === entry.liabilityId);
        const liabilityType = liability?.type ? liability.type.toUpperCase() : 'LIABILITY';
        const categoryLabel =
          liability?.type === 'emi' || liability?.type === 'rent'
            ? `${liabilityType} - ${monthKeyToLabel(entry.monthKey)}`
            : liabilityType;
        return {
          id: entry.id,
          amount: entry.amount,
          title: liability?.title || 'Liability Payment',
          category: categoryLabel,
          createdAt: entry.createdAt,
        };
      }),
    [dismissedRecentPaymentIds, liabilities, liabilityPayments]
  );
  const displayedLiabilityRows = useMemo(
    () => (showAllLiabilityPayments ? liabilityPaymentRows : liabilityPaymentRows.slice(0, 3)),
    [liabilityPaymentRows, showAllLiabilityPayments]
  );
  const groupedLiabilityRows = useMemo(() => {
    const groups: Record<string, typeof displayedLiabilityRows> = {};
    displayedLiabilityRows.forEach((row) => {
  ...
  }, [displayedLiabilityRows]);
  const removeRecentPaymentRow = (id: string) => {
        ...
        <View style={styles.transactionsContainer}>
          <View style={styles.transactionsHeader}>
            <View style={styles.sectionTitleRow}>
              ...
              <Text style={styles.recentHeaderTitle}>Recent Liability Payments</Text>
            </View>
            <TouchableOpacity style={styles.viewAllToggle} onPress={() => setShowAllLiabilityPayments((prev) => !prev)}>
              <Text style={styles.viewDayText}>{showAllLiabilityPayments ? 'Hide' : 'View All'}</Text>
              <Ionicons
                name={showAllLiabilityPayments ? 'chevron-up' : 'chevron-down'}
                size={16}
                color={colors.highlight}
              />
            </TouchableOpacity>
          </View>
          {groupedLiabilityRows.length === 0 ? (
            <Text style={styles.recentEmptyText}>No liability payments found.</Text>
          ) : (
            groupedLiabilityRows.map((group) => (
              <View key={group.key} style={styles.recentGroup}>
                ...
                {group.items.map((row) => (
                  <Swipeable
                    ...
                    renderRightActions={() => (
                      <TouchableOpacity style={styles.recentRemoveAction} onPress={() => removeRecentPaymentRow(row.id)}>
                  ...
                  >
                    <View style={styles.recentItemCard}>
                      <View style={styles.recentItemRow}>
                        ...
                        <Text style={styles.recentItemAmount}>-Rs {formatCurrency(row.amount)}</Text>
                      </View>
                      <Text style={styles.recentItemMeta}>Category: {row.category}</Text>
                    </View>
                  </Swipeable>
                ))}
              </View>
